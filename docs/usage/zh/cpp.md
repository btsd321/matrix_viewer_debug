...
- **只有同类型的面板才能同步** —— 下拉框中只列出类型匹配的面板（图像 / 折线图 / 点云）。
- **可以同步两个以上的面板。** 加入第三个面板会扩展同步组；连接两个已有的组会将其合并。组内成员会在工具栏和 **MatrixViewer Debug** 侧边栏中显示 `⇄1`、`⇄2` … 标记。
- 图像同步缩放与平移，折线图同步 X/Y 可见数据范围，点云同步相机位置与目标点。折线图的 **Histogram** 模式不参与同步。
- 侧边栏提供相同的操作：右键变量 → **Sync with…** / **Unsync**。
- 调试会话结束时会清空所有同步组。将 `matrixViewer.sync.showToolbarControls` 设为 `false` 可只使用侧边栏菜单。
