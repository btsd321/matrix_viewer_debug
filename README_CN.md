...
| `matrixViewer.image.compression.mode` | `string` | `"auto"` | 控制图像像素数据在发送给查看器前是否进行 zlib 压缩。`auto` — 仅在远程环境（Remote SSH、WSL、Dev Container）中压缩；`always` — 只要超过阈值就始终压缩；`never` — 永不压缩。|
