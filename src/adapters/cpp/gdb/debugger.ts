/**
 * gdb/debugger.ts — DAP communication layer for GDB (session.type = "cppdbg").
 *
 * Provides GDB-specific:
...
/** GDB (cppdbg) uses the "repl" evaluate context. */
 ...
 * the GDB "repl" evaluation context.
...
export async function getVariablesInScope(
    ...
    try {
        ...
        const resolved = await Promise.all(
            raw.map(async (v) => {
                let typeName = v.type ?? "";
                // GDB may omit type for some variables — try evaluate as fallback
...
/**
 * Build GDB-specific expressions to obtain a data pointer for a container
 * (std::vector, std::array, cv::Mat, raw pointers, etc.).
 * GDB uses (long long) casts to convert pointers to integers.
 */
...
/**
 * Build GDB-specific expressions to obtain the address of `varName[0][0]`.
...
/**
 * Build GDB-specific expressions to obtain the address of `varName[0][0][0]`.
...
/**
 * Evaluate `.size()` on a container variable using GDB/cppdbg cast syntax.
 ...
 *   2. Fall back to evaluating `.data()` with GDB-specific cast syntax
 */
